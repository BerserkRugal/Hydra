...
impl Proof {
    ...
    pub(crate) fn is_temp_proof(self, node: Digest, prooftype: ProofType) -> bool {
      if self.node == node{
...
impl BlockTree {
    ...
    fn get_parent_key_block(&self, c_block: Digest) -> Digest {
        let mut current = self.get(c_block).unwrap();

        ...
        for (i, pair) in ret.iter().enumerate() {
            if i == 0 && self.index == 0 {
                content.push_str(&format!(
                    "./hydra --config {} --export-path result-{}.json &\n",
                    pair.0.display(),
                    result_label
                ));
            } else {
                content.push_str(&format!(
                    "./hydra --config {} --disable-metrics &>/dev/null &\n",
                    pair.0.display()
                ));
            }
...
impl DistributionPlan {
    pub(crate) fn new(
        ...
        let initial_members: BTreeMap<_, _> = peer_addrs.iter().take(inum).map(|(&k, &v)| (k, v)).collect();
        let l_set: BTreeMap<_, _> = peer_addrs.iter().take(lnum).map(|(&k, &v)| (k, v)).collect();
    ...
    fn new_run_scripts(&self, hosts: &Vec<String>) -> Result<Vec<(PathBuf, String)>> {
        ...
        for host in hosts {
            ...
            content.push_str(&format!("ssh {} 'killall hydra' &\n", host));
